...
class Board(object):
    ...
    def __init__(self):
        #self._board = '-'*9 # 坑！！
        self._board = ['-' for _ in range(row*row)]
        self._history = [] # 棋谱
    ...
    def teminate(self):
        board = self._board
        line1 = [board[i*row:i*row+row] for i in range(row) ]
        line2 = [board[i::row] for i in range(row)]
        line3 = [board[0::row + 1]]
        line4 = [board[row - 1:row * row - row + 1:row - 1]]
...
class HumanPlayer(Player):
    ...
    def think(self, board):
        while True:
            say = "Please input a num in 0-%d" % (row*row-1)
            action = input(say)
            ra = [i for i in range(row*row-1)]

            if int(action) in ra and board.is_legal_action(int(action)):
                return int(action)
...
class AIPlayer2(Player):
    ...
    def think(self, board):
        ...
        player = AIPlayer2(take)     # 假想敌！！！
        _, action = self.minimaxcut(board, player)
        print('OK')
    ...
    # 极大极小法搜索，α-β剪枝
    def minimaxcut(self, board, player, depth=0, fatherbeta = 0, fatheralpha = 0) :
        ...
        for action in board.get_legal_actions() : # 遍历合法走法
            if self.take == "O" :
                ...
                if fatherbeta < alpha :
                    return
            board._move(action, self.take)
            val, _ = player.minimaxcut(board, self, depth+1,beta,alpha) # 切换到 假想敌！！！
            board._unmove(action) # 撤销走法，回溯
